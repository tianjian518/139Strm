...
#
# 【v2.2.16】60 秒 → 600 秒。这台服务器可能在海外，每次 list 根目录
# 都要跨国际线路走一趟；而「用户恰好在 10 分钟内删掉临时目录」这种事
# 概率极低，且真发生了也有兜底（秒传失败 → 强制重查重试），不会卡死。
#
# 【v2.2.16】600 秒 → 1800 秒。用户实测一个来回约 1 秒，这一秒不该
...
class CASRestorer:
    ...
    def parse(self, file_id, cas_name):
        ...
        【v2.2.16】缓存改为**落盘**：以前只在内存里，容器一重启或隔夜
        就全没了，续播又要重新付这两次往返 —— 而服务器如果在海外，
        这两次就是好几秒。这两秒是纯浪费，因为它取的东西永远不变。

        【v2.2.16】加**按文件的解析锁**：以前这里是 check-then-act，
